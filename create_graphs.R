...
get_stats_diversity <- function(rec_data, beta_rate){
  df <- rec_data %>% 
    filter(beta == beta_rate) %>%
...
# graph w/ themes
graph_stats_diversity <- function(df,beta){
  g <- ggplot(df2, aes(x=rho, y=diversity_mean)) +
    ...
    labs(x="rho", y="average diversity",
         title=paste("Beta=",beta,"N=500,T=25, Diversity",sep=" "),
...
get_stats_welfare <- function(rec_data, beta_rate){
  df <- rec_data %>% 
    filter(beta == beta_rate) %>%
    group_by(regime, pop_idx, rho, beta) %>% 
...
# graph w/ themes
graph_stats_welfare <- function(d,beta){
  g <- ggplot(d, aes(x=rho, y=welfare_mean)) +
    ...
    labs(x="rho", y="average welfare",
         title=paste("Beta=",beta,"N=500,T=25, Welfare",sep=" "),
         #subtitle="A plot that is only useful for demonstration purposes",
         caption="Columbia University") + 
    theme_ipsum_rc()
  return(g)
...
g <- graph_stats_welfare(get_stats_welfare(rec_data,.9), .9)
g
...
for (beta in c(0.9, 0.5, 0.1)){
  # diversity
  beta_name <- paste("beta_", beta, sep="")
  print(beta_name)
  df2 <- get_stats_diversity(rec_data, beta)
  g <- graph_stats_diversity(df2,beta)
  ggsave(filename=paste(WORKING_DIR, "figures2/", beta_name, "_diversity.jpeg", sep=""), plot=g)
  
  #welfare
  g <- graph_stats_welfare(get_stats_welfare(rec_data,beta), beta)
  ggsave(filename=paste(WORKING_DIR, "figures2/", beta_name, "_welfare.jpeg", sep=""), plot=g)
