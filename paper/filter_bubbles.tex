...
\begin{abstract}
We study a model of user decision-making in the context of recommender systems via numerical simulation. Our model provides an explanation for the findings of Nguyen, et. al (2014), where users consume increasingly similar items over time even without recommendation. Our model naturally generates such filter\dgedit{-}bubble effects since users face a sequential decision making problem under uncertainty and there are informational spillovers where a user consuming an item and learning how she values it reduces uncertainty and provides information about similar items.
\par
In line with Nguyen, et. al (2014), recommendation alleviates these filter\dgedit{-}bubble effects. However, we find that it also leads to an increase in homogeneity across users, resulting in a trade-off between homogenizing across user consumption and diversifying
within user consumption. Finally, our model highlights the importance of collecting data on user beliefs and their evolution over time both to design better recommendations and to further understand their impact.
\end{abstract}
...
\xhdr{An Illustrative Example\dgedit{.}} We illustrate the main intuitions of our model with a simple example. Suppose that there are four items: 0, 1, 2, 3. The items are in different places of the product space, where 0 is close to 1 and 3 but more distant from 2. \dgedit{For the sake of expositional clarity, suppose that t}he initial beliefs are given as follows:
...
\dgedit{Our first finding can be summarized as follows:}
...
\begin{figure}[t]
\caption{Relationship between Local Consumption and $\rho$, $\gamma$}
...
\begin{enumerate}
\item Under oracle recommendation, welfare is \dgedit{invariant with respect to }$\rho$.
\item Under no recommendation, welfare is increasing \dgedit{in} $\rho$.
...
\begin{figure}[t]
\caption{Relationship between $\rho$, $\gamma$ and User Welfare, Diversity}\label{fig:diversity_welfare_correlation}
...
\dgdelete{Finding~\ref{finding_welfare_gap} states our main findings of the impact of recommendation on ex-post welfare, which can be seen in the rightmost plot of~\autoref{fig:diversity_welfare_correlation}. }
