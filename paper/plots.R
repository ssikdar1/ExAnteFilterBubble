...
graph_stats_diversity <- function(df, var, use_hrbrthemes){
  ...
  if(use_hrbrthemes){
    g <- g + theme_bw() + theme(axis.title.y=element_text(size=16, face="bold"), axis.title.x=element_text(size=16, face="bold"))
  }
  return(g)
}
  ...
  if (var == "rho") {
    g <- ggplot(df, aes(x=rho, y=jaccard_mean)) +
      ...
      labs(x="rho", y="homogeneity",
           title=paste("N =", N, "T =", T_val, "Homogeneity",sep=" ")) + theme_bw() + theme(axis.title.y=element_text(size=16, face="bold"), axis.title.x=element_text(size=16, face="bold"))
    return(g)
  } else if (var == "beta") {
    g <- ggplot(df, aes(x=beta, y=jaccard_mean)) +
      ...
      labs(x="beta", y="homogeneity",
           title=paste("Homogeneity",sep=" ")) + theme_bw() + theme(axis.title.y=element_text(size=16, face="bold"), axis.title.x=element_text(size=16, face="bold"))
    return(g)
  } else if (var == "sigma") {
...
rec_data <- rec_data %>% mutate(diversity_score = diversity_score / 1000)
...
g <- g + theme_bw() + theme(legend.position="bottom", legend.title = element_blank(), axis.title.y=element_text(size=16, face="bold"))  + guides(color=guide_legend(override.aes=list(fill=NA)))
...
if(use_hrbrthemes){
  g <- g + theme_bw() + theme(axis.title.y=element_text(size=16, face="bold"))
}
ggsave(
